                ...
                if ($valid) {
                    ...
                    if ($this->generateName) {
                        if ($this->attributeForName !== null) {
                            $filename = $this->prefix . Inflector::slug($this->owner->{$this->attributeForName}) . '_' . substr(md5_file($file->tempName), 0, 5) . '.' . ($this->forceExt ? $this->forceExt : $file->getExtension());
                        }
                    ...
                    if (!$valid) {
                        ...
                        if (preg_match('#\b(([\w-]+://?|www[.])[^\s()<>]+(?:\([\w\d]+\)|([^[:punct:]\s]|/)))#i', $fileName, $matches)) {
                            ...
                            if ($content) {
                                ...
                                if ($this->generateName) {
                                    if ($this->attributeForName !== null) {
                                        $newFileName = $this->prefix . Inflector::slug($this->owner->{$this->attributeForName}) . '_' . substr(md5($content), 0, 5) . '.' . $ext;
                                    }
